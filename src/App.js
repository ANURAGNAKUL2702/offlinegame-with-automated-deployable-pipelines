  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <button className="game-button-container" onClick={() => setShowGame(true)}>
...
const DinoGame = ({ onExit }) => {
  ...
  useEffect(() => {
    ...
    function jump() {
      if (!dino.jumping && !gameOver) {
        ...
        dino = { x: 50, y: 150, width: 30, height: 50, jumping: false, velocity: 0 };
        obstacles = [];
        score = 0;
        gameSpeed = 5;
        gameOver = false;
  ...
  return (
    <div style={{ textAlign: 'center', padding: '20px' }}>
      <button className="exit-button" onClick={onExit}>← Back to App</button>
      <canvas 
        ref={canvasRef} 
